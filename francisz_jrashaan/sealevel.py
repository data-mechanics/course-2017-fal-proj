...
class seaLevel(dml.Algorithm):

     contributor = 'francisz_jrashaan'

     reads = []

     writes = ['francisz_jrashaan.seaLevel']
     ...
     def execute(trial = False):
         ...
         url = 'Users/Francis/desktop/keywest.json'
         response = urllib.request.urlopen(url).read().decode("utf-8")
         r = json.loads(response)
         s = json.dumps(r, sort_keys=True, indent=2)


         repo.dropPermanent("seaLevel")

         repo.createPermanent("seaLevel")
         ...
         x =  repo.francisz_jrashaan.seaLevel.find()
         print(x)
                                 
       
         
         
         for entry in repo.francisz_jrashaan.seaLevel.find():
