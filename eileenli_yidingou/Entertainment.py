...
class example(dml.Algorithm):
    ...
    reads = []
    writes = ['eileenli_yidingou.Entertainment']
    ...
    def execute(trial = False):
        ...
        url = 'http://datamechanics.io/data/eileenli_yidingou/new.json'
        response = urllib.request.urlopen(url).read().decode("utf-8")
        r = json.loads(response)
        s = json.dumps(r, sort_keys=True, indent=2)
        repo.dropCollection("Entertainment")
        repo.createCollection("Entertainment")
        repo['eileenli_yidingou.Entertainment'].insert_many(r)
        repo['eileenli_yidingou.Entertainment'].metadata({'complete':True})
        print(repo['eileenli_yidingou.Entertainment'].metadata())
    ...
    def provenance(doc = prov.model.ProvDocument(), startTime = None, endTime = None):
        ...
        Entertainment = doc.entity('dat:eileenli_yidingou#Entertainment', {prov.model.PROV_LABEL:'NightLife Entertainment', prov.model.PROV_TYPE:'ont:DataSet'})
