    ...
    def execute(trial=False):
        ...
        def select311TrashDel(dictList):
            ...
            for each_dict in dictList:
                wanted_columns = ['neighborhood', 'Latitude',
                                  'Longitude']
                adict = dict((k, each_dict[k]) for k in wanted_columns
                             if k in each_dict)
        ...
        def selectBigBellyDel(dictList):
        	newDictList = []
        	for each_dict in dictList:
        		wanted_columns = ["fullness", "collection", "Location"]
        		adict = dict((k, each_dict[k]) for k in wanted_columns if k in each_dict)
        		newDictList.append[adict]

        	return newDictList

        selectBigBellyReduced = selectBigBellyDel(selectBigBellyGPS)
