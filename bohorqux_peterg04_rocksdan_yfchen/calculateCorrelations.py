...
#         print(finalData)
    ...
    def provenance(doc = prov.model.ProvDocument(), startTime = None, endTime = None):
        ...
        this_script = doc.agent('alg:bohorqux_peterg04_rocksdan_yfchen#calculateCorrelations', {prov.model.PROV_TYPE:prov.model.PROV['SoftwareAgent'], 'ont:Computation':'py'})
        resource_propertycrimes = doc.entity('dat:bohorqux_peterg04_rocksdan_yfchen#property_crimes', {'prov:label':'MongoDB', prov.model.PROV_TYPE:'ont:DataResource'})
        calcCorr = doc.activity('log:uuid'+str(uuid.uuid4()), startTime, endTime)
        doc.wasAssociatedWith(calcCorr, this_script)
        doc.usage(calcCorr, resource_propertycrimes, startTime, None,
                  {prov.model.PROV_TYPE:'ont:Retrieval',
                  'ont:Query':'?type=calculateCorrelations&$select=crimes_lowPropertyDensity,crimes_highPropertyDensity'
                  }
                  )
 
        calcedCorr = doc.entity('dat:bohorqux_peterg04_rocksdan_yfchen#calculateCorrelations', {prov.model.PROV_LABEL:'Calculated Correlations', prov.model.PROV_TYPE:'ont:DataSet'})
        doc.wasAttributedTo(calcedCorr, this_script)
        doc.wasGeneratedBy(calcedCorr, calcCorr, endTime)
        doc.wasDerivedFrom(calcedCorr, resource_propertycrimes, calcCorr, calcCorr, calcCorr)
